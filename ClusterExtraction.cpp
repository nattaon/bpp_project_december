...
{
	cloud_cluster_no_filter.reset(new PointCloudXYZRGB);
	pcl::copyPointCloud(*cloud, *cloud_cluster_no_filter);
	...
	for (std::vector<pcl::PointIndices>::const_iterator it = vector_cluster_indices.begin(); it != vector_cluster_indices.end(); ++it)
	{
		...
		for (std::vector<int>::const_iterator pit = it->indices.begin(); pit != it->indices.end(); ++pit)
		{
			...
			cloud_cluster->points.push_back(cloud_cluster_no_filter->points[*pit]);
...
void ClusterExtraction::ShowClusterBoundingbox(PointCloudXYZRGB::Ptr cloud)
{
	cloud_cluster_no_filter.reset(new PointCloudXYZRGB);
	pcl::copyPointCloud(*cloud, *cloud_cluster_no_filter);
